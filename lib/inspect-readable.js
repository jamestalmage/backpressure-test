...
function inspectReadable(input) {
	...
	function validateChunk(chunk, i) {
		...
		var compareTo = chunksArray[i];
		if (Buffer.isBuffer(compareTo) && typeof chunk === 'string') {
			chunk = new Buffer(chunk);
		}
		if (Buffer.isBuffer(compareTo)) {
			assert(Buffer.isBuffer(chunk), 'stream contains buffers, but tried to validate a: ' + (typeof chunk));
			if (!compareTo.equals(chunk)) {
				assert.fail(compareTo.toString(), chunk.toString(), 'invalid chunk at ' + i);
			}
		} else {
			assert(!Buffer.isBuffer(chunk), 'stream contains non-buffers, but tried to validate a buffer');
			assert.deepEqual(compareTo, chunk, 'invalid objectMode chunk at: ' + i);
		}
		return true;
	}
